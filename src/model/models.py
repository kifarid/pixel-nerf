...
class DNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        """
        :param conf PyHocon config subtree 'model'
        """
        super().__init__()
        self.use_xyz = conf.get("use_xyz", False)
        assert self.use_xyz  # Must use some feature..
        self.stop_encoder_grad = (
            stop_encoder_grad  # Stop ConvNet gradient (freeze weights)
        )
        self.use_code = conf.get("use_code", False)  # Positional encoding
        ...
        self.global_encoder = make_encoder(conf["global_encoder"]) #ImageEncoder.from_conf(conf["global_encoder"])
        self.global_encoder_latent_size = self.global_encoder.latent_size
        ...
        self.flow_net = make_mlp(conf["mlp_flow"], 3, self.global_encoder_latent_size)
    ...
    def encode(self, images, poses, focal, c=None):
        """
        :param images (NS, 3, H, W) or (N, NS, 3, H, W)
        NS is number of input (aka source or reference) views
        :param poses (NS, 4, 4)
        :param focal focal length () or (2) or (NS) or (NS, 2) [fx, fy]
        :param z_bounds ignored argument (used in the past)
    ...
    def get_posi_encoder(self, xyz, viewdirs):
        """
        This function takes xyz in world coordinates and returns the latent
        representation of the position of the points in the camera plane.
        :param xyz (SB, B, 3)
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            # Concat global latent code if enabled
            global_latent = self.get_latent()
            print(global_latent.size(), "latent size")
            if self.stop_encoder_grad:
                global_latent = global_latent.detach()

            xyz_flow_in = xyz.reshape(-1, 3)
            assert xyz_flow_in.shape[0] % global_latent.shape[0] == 0
            num_repeats = xyz_flow_in.shape[0] // global_latent.shape[0]
            global_latent = repeat_interleave(global_latent, num_repeats)
            print(global_latent.shape, xyz_flow_in.shape, "global latent and xyz flow in")
            ...
            #xyz_shifted = torch.clamp(xyz_shifted, -2, 2)
            mlp_input = self.get_posi_encoder(xyz_shifted, viewdirs)
            #mlp_input = torch.cat((global_latent, mlp_input), dim=-1)
            # Run main NeRF network
            if coarse or self.mlp_fine is None:
                mlp_output = self.mlp_coarse(
