...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            vic_radius = self.vic_radius if coarse else self.vic_radius/2

            if vic_radius > 0:
                # Generate random noise from a normal distribution with mean 0 and standard deviation 0.1
                pnts_sz = points.size()
                assert pnts_sz[0] % 2 == 0, "Number of points should be even"
                #make sure that we get an even number of points
                if pnts_sz[0] % 2 != 0:
                    pnts_sz[0] -= 1
                
                points_vic = points + torch.randn(*pnts_sz).to(points) * vic_radius
                
                # # get a shuffled version of the points
                # points_shuffled = points[torch.randperm(pnts_sz[0])]
                # points_vic= points_shuffled + 0.5*torch.randn(*pnts_sz).to(points)
                # # last 50% of the points_vic is a perturbed version of random points from points (size of 50%)
                # vic_pnts_sz = (int(pnts_sz[0]/2), pnts_sz[1])
                # points_vic[-int(pnts_sz[0]/2):] = points_vic[:int(pnts_sz[0]/2)] + torch.randn(vic_pnts_sz).to(points) * vic_radius
            ...
            if use_viewdirs:
                ...
                if self.vic_radius > 0:
                    for pnts_vic, dirs in zip(split_points_vic, split_viewdirs):
                        model_vic_o = model(pnts_vic, coarse=coarse, viewdirs=dirs)
                        val_all_vic.append(model_vic_o)
                ...
                if self.vic_radius > 0:
                    for pnts_vic in split_points_vic:
                        model_vic_o = model(pnts_vic, coarse=coarse)
                        val_all_vic.append(model_vic_o)
            ...
            if self.vic_radius > 0:
                out_vic = torch.cat(val_all_vic, dim=eval_batch_dim).reshape(B, K, -1)
                rgb_diff, depth_diff = self.get_diff(out_vic, out)
                # out_vic = torch.cat(val_all_vic, dim=eval_batch_dim).reshape(B*K, -1)
                # out_vic_1 = out_vic[:int(out_vic.size()[0]/2)]
                # out_vic_2 = out_vic[int(out_vic.size()[0]/2):]
                # rgb_diff, depth_diff = self.get_diff(out_vic_1, out_vic_2)
                return (weights, rgb_final, depth_final), (rgb_diff, depth_diff)
    ...
    def get_diff(self, out, out_target):
        """return the difference in the rgb and depth values between the two outputs"""
        rgbs = out[..., :3]
        alphas = out[..., 3]

        rgbs_target = out_target[..., :3]
        alphas_target = out_target[..., 3]

        #RMSE loss
        
        rgb_diff = F.mse_loss(rgbs, rgbs_target.detach())
        alphas = out[..., 3]
        alphas_target = out_target[..., 3]
        alphas_diff = F.mse_loss(alphas, alphas_target.detach())
        return rgb_diff, alphas_diff
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            if self.vic_radius > 0:
                outputs.coarse.rgb_vic_diff = coarse_composite_vic[0]
                outputs.coarse.depth_vic_diff = coarse_composite_vic[1]
            ...
            if self.using_fine:
                ...
                if self.vic_radius > 0:
                    outputs.fine.rgb_vic_diff = fine_composite_vic[0]
                    outputs.fine.depth_vic_diff = fine_composite_vic[1]
    ...
    def from_conf(cls, conf, white_bkgd=False, lindisp=False, eval_batch_size=100000):
        return cls(
            ...
            vic_radius=conf.get("vic_radius", 0.0),            
        )
...
# class DNeRFRenderer(NeRFRenderer):
    ...
    #             # Generate random noise from a normal distribution with mean 0 and standard deviation 0.1
    #             pnts_sz = points.size()
    #             noise = torch.randn(*pnts_sz).to(points) * self.vic_radius
    ...
    #         out = torch.cat(val_all, dim=eval_batch_dim)
    #         out = out.reshape(B, K, -1)  # (B, K, 4 or 5)
    #         print(val_all[0].shape, val_all_flow[0].shape, "more shapes")
