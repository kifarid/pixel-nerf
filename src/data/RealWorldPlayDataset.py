...
class RealWorldPlayDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, directory, stage="train", z_near=0.01, z_far=2, frame_rate=1, frame_stack=1,
        ...
        # root, dirs, files = os.walk(self.directory)
        # subdirectory_paths = sorted([os.path.join(root, f) for f in files if f.startswith('episode_')])
...
if __name__ == "__main__":
    frame_stack = 10
    dataset = RealWorldPlayDataset(
        "/home/huang/hcg/projects/nerf_manipulation/data/500k_all_tasks_dataset_15hz/",
        frame_stack=2,
        frame_rate=3,
        views=[1, 2],
        relative=True,
